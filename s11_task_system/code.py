...
"""s11 — Task System
    ...
    ┌───────────────────┐          ┌──────────────────────────┐
    │ "好的，我分三步：   │          │ #1 替换 print   completed │
    │  ① 替换 print      │   ✗ 会   │ #2 加类型标注   in_progress│
    │  ② 加类型标注      │   被压缩  │ #3 跑测试       pending    │
    │  ③ 跑测试"         │   稀释    │    depends_on: [1, 2]     │
    └───────────────────┘          └──────────────────────────┘
       一段会过期的文字                 每一步都重新渲染进 prompt

这一章回答：**计划应该放在模型脑子里，还是放在 Harness 手上？**

s10 的压缩会把"我打算做三件事"那句话遮蔽掉。摘要提示词里写了
"保留还没做完的事"，但那是在**祈祷**模型每次都写对，不是**保证**。

    Task 是 Harness 的外部状态，不应该只存在模型的上下文里。
...
def build_big_workspace() -> Path:
    """一个需要多步骤改造的小项目。"""
    d = Path(tempfile.mkdtemp(prefix="s11_demo_"))
    (d / "core.py").write_text(
        "\n".join(["def load(path):", "    print('loading', path)", "    return open(path).read()", ""] +
                  [f"def helper_{i}(x):\n    print('helper {i}')\n    return x + {i}\n" for i in range(18)]),
        encoding="utf-8")
    (d / "cli.py").write_text(
        "\n".join(["import core", "", "def main():", "    print('start')", "    core.load('a.txt')",
                   "    print('done')", ""] +
                  [f"OPT_{i} = '{i}'" for i in range(30)]), encoding="utf-8")
    (d / "test_core.py").write_text("def test_smoke():\n    assert True\n", encoding="utf-8")
    return d
...
def print_tasks(store: TaskStore) -> None:
    ...
    tasks = store.current()
    print(f"\n\033[1m任务清单（Harness 状态，不在模型脑子里）\033[0m")
    for t in tasks:
...
def demo(debug: bool) -> None:
    global WORKSPACE, SKILLS, INBOX, RT, PROVIDER_FOR_SUBAGENT, PARENT_SESSION, TRACER, SUMMARIZER, TASKS
    ...
    SUMMARIZER = get_provider(demo_script=[
        scripted("目标：把 core.py / cli.py 的 print 换成 logging，然后跑测试。\n"
                 "已完成：core.py 的 print 已替换。\n未完成：cli.py 尚未处理；测试未跑。"),
        scripted("目标同上。已完成：core.py 与 cli.py 的 print 均已替换。未完成：跑测试验证。"),
        scripted("目标同上。已完成：两处替换。未完成：跑测试验证。"),
    ])

    q = "把这个项目里所有 print 换成 logging，然后跑测试确认没坏。"
    print(f"\033[36m你 > \033[0m{q}")
    INBOX.put(q)

    script = [
        # ① 先规划 —— 是模型自己决定要规划的，Harness 没有强制
        scripted(calls=[("task_write", {"tasks": [
            {"id": "t1", "title": "把 core.py 的 print 换成 logging", "status": "in_progress"},
            {"id": "t2", "title": "把 cli.py 的 print 换成 logging", "status": "pending"},
            {"id": "t3", "title": "跑测试确认没坏", "status": "pending", "depends_on": ["t1", "t2"]},
        ]})]),
        # ② 故意演示一次校验失败：t3 的前置还没完成就想标 completed
        scripted(calls=[("task_write", {"tasks": [
            {"id": "t1", "title": "把 core.py 的 print 换成 logging", "status": "in_progress"},
            {"id": "t2", "title": "把 cli.py 的 print 换成 logging", "status": "pending"},
            {"id": "t3", "title": "跑测试确认没坏", "status": "completed", "depends_on": ["t1", "t2"]},
        ]})]),
        scripted(calls=[("read", {"path": "core.py"})]),
        scripted(calls=[("edit", {"path": "core.py", "old_text": "def load(path):\n    print('loading', path)",
                                  "new_text": "import logging\n\ndef load(path):\n    logging.info('loading %s', path)"})]),
        scripted(calls=[("task_write", {"tasks": [
            {"id": "t1", "title": "把 core.py 的 print 换成 logging", "status": "completed",
             "note": "load() 已改；helper_* 仍有 print，属于后续清理"},
            {"id": "t2", "title": "把 cli.py 的 print 换成 logging", "status": "in_progress"},
            {"id": "t3", "title": "跑测试确认没坏", "status": "pending", "depends_on": ["t1", "t2"]},
        ]})]),
        ...
        scripted(calls=[("task_write", {"tasks": [
            {"id": "t1", "title": "把 core.py 的 print 换成 logging", "status": "completed",
             "note": "load() 已改"},
            {"id": "t2", "title": "把 cli.py 的 print 换成 logging", "status": "completed"},
            {"id": "t3", "title": "跑测试确认没坏", "status": "in_progress", "depends_on": ["t1", "t2"]},
        ]})]),
        scripted(calls=[("bash", {"command": "python3 -c \"import core, cli; print('smoke ok')\""})]),
        scripted(calls=[("task_write", {"tasks": [
            {"id": "t1", "title": "把 core.py 的 print 换成 logging", "status": "completed",
             "note": "load() 已改"},
            {"id": "t2", "title": "把 cli.py 的 print 换成 logging", "status": "completed"},
            {"id": "t3", "title": "跑测试确认没坏", "status": "completed",
             "depends_on": ["t1", "t2"], "note": "smoke ok"},
        ]})]),
        scripted("三项都完成了：core.py 和 cli.py 的 print 已换成 logging，冒烟检查通过。"),
    ]
    ...
    # ── 关键证明：压缩之后清单还在 ───────────────────────────────
    print("\n\033[1m【关键】压缩发生了，但任务清单毫发无损\033[0m")
    n_compact = sum(1 for e in session.events() if e.type == EV_COMPACTION_SUMMARY)
    _, shadowed = collect_shadows(session)
    print(f"  这一轮压缩了 {n_compact} 次，遮蔽了 {len(shadowed)} 条消息")
    print(f"  但 task/write 是 log-only，压缩碰不到它：")
    writes = [e for e in session.events() if e.type == EV_TASK_WRITE]
    print(f"    日志里有 {len(writes)} 条 task/write 快照，全部完好")
    print(f"    当前清单从最后一条快照（#{writes[-1].seq}）重新渲染")
    section = _tasks(RT) or ""
    print(f"    它每一步都作为 prompt 的一块出现（{len(section)} 字符）")
    print("\033[90m  对比一下：模型在第 1 步说的「我打算做三件事」是一条 assistant 消息，")
    print("  它已经被压缩遮蔽了。如果计划只存在那句话里，现在就丢了。\033[0m")

    # ── 一致性校验 ───────────────────────────────────────────────
    print("\n\033[1m【校验】Harness 管状态自洽，不管清单内容\033[0m")
    for ev in session.events():
        if ev.type == EV_TOOL_RESULT and ev.data["name"] == "task_write" \
                and ev.data["content"].startswith("错误"):
            print(f"  \033[31m✗\033[0m {ev.data['content']}")
    print("  \033[90m（模型想在前置未完成时就把 t3 标 completed，被挡回去了）\033[0m")
    print("  \033[90mHarness 管：id 重复 / 依赖不存在 / 依赖成环 / 前置未完成就标完成")
    print("  模型管：清单里该有哪些任务、怎么拆、先做哪个\033[0m")

    print("\n\033[90m" + "─" * 68)
    print("Task 是 Harness 的外部状态，不是模型上下文的一部分。")
    print("所以它扛得住压缩、扛得住多轮稀释、进程重启后还能从日志恢复。\033[0m")
...
def main() -> None:
    ...
    print("\033[1ms11 — Task System\033[0m")
    print(f"\033[90m日志 {log_path}；/tasks 看清单，/ctx 看上下文，q 退出\033[0m\n")
    ...
    while True:
        ...
        print(f"\033[90m[turn {out.turn} · {out.steps} steps · 任务 {done}/{len(tasks)} · "
