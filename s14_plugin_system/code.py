...
这一章回答：**"Everything is a plugin" 到底解决了什么？**

不是为了炫技。是为了让一个功能有**边界**：

    Replaceable   换掉权限策略 = 换一个 plugin
    Composable    只读版 Agent = 少 use 几个 plugin
    Extensible    加新能力 = 写一个 plugin，不碰任何已有代码

关键机制只有一条：**注册是可逆的 effect**。
每次注册都返回一个撤销函数，plugin 卸载时逆序执行。

运行：
    python s14_plugin_system/code.py --demo
    python s14_plugin_system/code.py --demo --debug
    python s14_plugin_system/code.py --profile readonly
"""
...
class CoreToolsPlugin:
    ...
    注意工具现在是**闭包**，捕获 `ctx` 而不是读全局变量。
    s13 里 run_bash 要读全局 WORKSPACE，所以它不可能同时服务两个工作区；
    现在它捕获的是 plugin 自己的 cwd，一个进程里可以有多套。
    """
    ...
    def setup(self, ctx: PluginContext) -> None:
        ...
        def _bash(command: str) -> str:
            try:
                ...
                return "错误：命令超时（60 秒）。耗时长的命令请用 bash_background。"
        ...
        def _read(path: str, limit: int | None = None) -> str:
            lines = _safe_path(cwd, path).read_text(encoding="utf-8").splitlines()
        ...
        def _glob(pattern: str) -> str:
            hits = sorted(globlib.glob(pattern, root_dir=cwd, recursive=True))
        ...
        def _grep(pattern: str, glob: str = "**/*") -> str:
            hits: list[str] = []
            for name in sorted(globlib.glob(glob, root_dir=cwd, recursive=True)):
                f = cwd / name
                if not f.is_file():
                    continue
                try:
...
def build_harness(profile: str, cwd: Path, session: Session, tracer: Tracer,
                  ...
                  approver: Approver, skills_root: Path) -> Harness:
    """装配。
...
def demo(debug: bool) -> None:
    ...
    session = Session(path=cwd / "session.jsonl")
    session.append(EV_SESSION_START, {"cwd": str(cwd)})
    skills_root = Path(__file__).resolve().parent / "skills"

    h = build_harness("full", cwd, session, tracer,
                      provider_factory=lambda: get_provider(demo_script=[scripted("（未使用）")]),
                      summarizer=get_provider(demo_script=[scripted("（摘要略）")]),
                      approver=lambda *a: True, skills_root=skills_root)
    h.rt.project_notes = load_project_notes(cwd)
    executor = ToolExecutor(h.tools, h.bus)

    print("\033[1m【1】Harness 自己什么功能都没有，全靠插件\033[0m")
    print_harness(h)

    print("\n\033[1m【2】跑一轮，各插件协同工作\033[0m")
    q = "读一下 config.env，把 app.py 版本号升到 0.2.0，然后按 AGENTS.md 的要求验证。"
    print(f"\033[36m你 > \033[0m{q}")
    h.inbox.put(q)
    out = run_turn(get_provider(demo_script=[
        scripted(calls=[("task_write", {"tasks": [
            {"id": "t1", "title": "读 config.env", "status": "in_progress"},
            {"id": "t2", "title": "升版本号并验证", "status": "pending"}]})]),
        ...
        scripted(calls=[("task_write", {"tasks": [
            {"id": "t1", "title": "读 config.env", "status": "completed", "note": "密钥已脱敏"},
            {"id": "t2", "title": "升版本号并验证", "status": "completed", "note": "输出 0.2.0"}]})]),
        scripted("版本号已升到 0.2.0，验证输出 0.2.0。config.env 里的密钥是脱敏后看到的。"),
    ]), h, executor, tracer)
    print(f"\033[32m模型 >\033[0m {out.text}")

    # ── 卸载一个插件 ─────────────────────────────────────────────
    print("\n\033[1m【3】卸载一个插件：一行代码，四样东西一起消失\033[0m")
    before_tools, before_sections = set(h.tools.names()), set(h.prompts.names())
    before_services = set(h.services)
    before_listeners = sum(len(i) for _, _, i in h.bus.describe())

    h.unload("jobs")

    print(f"  工具：   -{sorted(before_tools - set(h.tools.names()))}")
    print(f"  prompt： -{sorted(before_sections - set(h.prompts.names()))}")
    print(f"  service：-{sorted(before_services - set(h.services))}")
    print(f"  监听器： {before_listeners} → {sum(len(i) for _, _, i in h.bus.describe())}")
    print("\033[90m  s13 里做同样的事要改 4 个地方，而且很容易漏掉 pre-step 里那个 pump_jobs()。\033[0m")

    print("\n\033[1m【4】卸载之后，模型确实拿不到那些工具了\033[0m")
    r = executor.execute("probe", "bash_background", {"command": "sleep 1"}, session, 99, 1)
    print(f"  \033[31m{r.content}\033[0m")

    # ── 换一个 profile ───────────────────────────────────────────
    print("\n\033[1m【5】换一份插件清单 = 换一个产品形态\033[0m")
    for profile in ("minimal", "readonly", "full"):
        s2 = Session()
        h2 = build_harness(profile, cwd, s2, Tracer(False),
                           lambda: get_provider(demo_script=[]),
                           get_provider(demo_script=[]), lambda *a: True, skills_root)
        print(f"  \033[36m{profile:<9}\033[0m {len(h2.loaded()):>2} 个插件  "
              f"{len(h2.tools.names()):>2} 个工具  {len(h2.prompts.names())} 个 prompt 段")
        if profile == "readonly":
            ex2 = ToolExecutor(h2.tools, h2.bus)
            rr = ex2.execute("p", "write", {"path": "x", "content": "y"}, s2, 1, 1)
            print(f"            \033[90m写操作：{rr.content[:52]}\033[0m")

    # ── 加一个插件，不碰任何已有代码 ─────────────────────────────
    print("\n\033[1m【6】加一个新功能：写一个 plugin，不碰任何已有代码\033[0m")

    class NotePlugin:
        """一个 30 行的新功能：给 Agent 一个便签本。"""

        name = "notes"

        def setup(self, ctx: PluginContext) -> None:
            notes: list[str] = []
            ctx.provide("notes", notes)

            @ctx.tool("note", "记一条便签，之后一直显示在你的上下文里。",
                      {"type": "object", "properties": {"text": {"type": "string"}},
                       "required": ["text"]})
            def _note(text: str) -> str:
                notes.append(text)
                return f"已记下（共 {len(notes)} 条）"

            @ctx.section("notes", 17)
            def _notes_section(rt: RuntimeContext) -> str | None:
                return "# 便签\n" + "\n".join(f"- {n}" for n in notes) if notes else None

    h.use(NotePlugin())
    executor.execute("n1", "note", {"text": "config.env 里的密钥不要写进日志"}, session, 99, 2)
    print(f"  新工具：note  新 prompt 段：notes")
    print(f"  prompt 里现在有：{h.prompts.assemble(h.rt).count('# 便签')} 段便签")
    print("\033[90m  没有改 Harness、没有改 run_turn、没有改任何别的插件。\033[0m")

    if debug:
        print_bus(h.bus)

    print("\n\033[90m" + "─" * 68)
    print("\"Everything is a plugin\" 的目的不是炫技，是让功能有**边界**：")
    print("  Replaceable   换权限策略 = 换一个 plugin")
    print("  Composable    只读版 Agent = 少 use 几个 plugin")
    print("  Extensible    加新能力 = 写一个 plugin，不碰已有代码")
    print("而这一切只靠一条机制：**注册返回撤销函数，卸载时逆序执行**。\033[0m")
...
def main() -> None:
    ...
    print(f"\033[1ms14 — Plugin System\033[0m  \033[90mprofile={profile}\033[0m")
    print_harness(h)
    print(f"\n\033[90m日志 {log_path}；/plugins /bus /unload <名字> /use-note，q 退出\033[0m\n")
    ...
    while True:
        ...
        if q.startswith("/unload "):
            ...
            h.unload(name)
            print(f"\033[90m已卸载 {name}\033[0m")
            print_harness(h)
            continue
        if q:
            h.inbox.put(q)
