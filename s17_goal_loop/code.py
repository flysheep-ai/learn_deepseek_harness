...
这一章回答：**"修好这个 bug" 这种目标，应该住在哪里？**

如果只住在模型的上下文里，那么 turn 结束就是真的结束 ——
没人会评估、没人会续跑、关掉终端目标就死了。

目标应该是 Harness 的**持久状态**：有生命周期、有预算、有 checkpoint。

注意分工：Harness 管「目标保存、状态保存、预算、停止条件」，
...
def demo(debug: bool) -> None:
    ...
    (cwd / "core.py").write_text(
        "\n".join(["def divide(a, b):", "    return a / b", "",
                   "def load(path):", "    return open(path).read()"]), encoding="utf-8")

    sess = Session(path=cwd / "session.jsonl")
    sess.append(EV_SESSION_START, {"cwd": str(cwd)})

    # 评估器有独立的脚本：三轮 continue 之后 done
    eval_verdicts = [
        scripted("verdict: continue\nreason: divide 还没有除零保护，模型这轮只是在读文件，没有改代码。"),
        scripted("verdict: continue\nreason: 模型改了一半（加了保护但语法错了），需要重跑验证。"),
        scripted("verdict: done\nreason: divide 已有除零保护且验证通过。"),
    ]

    h = build_harness("full", cwd, sess, tracer,
                      lambda: get_provider(demo_script=[scripted("（未使用）")]),
                      get_provider(demo_script=eval_verdicts),
                      lambda *a: True, skills_root)
    h.rt.project_notes = load_project_notes(cwd)
    executor = ToolExecutor(h.tools, h.bus)

    print("\033[1m【1】设定一个跨轮的目标\033[0m")
    goal = Goal(statement="修好 core.py 的 divide 对 b=0 的处理，并验证通过。", max_rounds=4)
    h.get("goal").set(goal)
    print(f"  \033[36mgoal/start\033[0m {goal.statement}  \033[90m（预算 {goal.max_rounds} 轮）\033[0m")
    print("  goal 的 prompt 段（active 时每步都在）：")
    print("\033[90m" + "  \n".join(_goal_text(h).splitlines()) + "\033[0m")

    print("\n\033[1m【2】第 1 轮：模型还没动手 —— 评估器说 continue\033[0m")
    h.inbox.put("修一下 divide 的除零问题")
    turn1 = [
        scripted(calls=[("read", {"path": "core.py"})]),
        scripted("我看了一下 divide，先想想怎么改。"),
    ]
    out = run_turn(get_provider(demo_script=turn1), h, executor, tracer)
    print(f"\033[32m模型 >\033[0m {out.text}")
    print(f"\033[90m→ turn {out.turn} 结束。挂在 turn-end 上的评估器判定：continue\033[0m")

    print("\n\033[1m【3】第 2 轮：模型改出了语法错误 —— 评估器说 continue\033[0m")
    turn2 = [
        scripted(calls=[("edit", {"path": "core.py",
                                  "old_text": "def divide(a, b):\n    return a / b",
                                  "new_text": "def divide(a, b):\n    if b == 0\n        return None\n    return a / b"})]),
        scripted("改好了。（其实漏了冒号 —— 评估器下一轮会抓住它）"),
    ]
    out = run_turn(get_provider(demo_script=turn2), h, executor, tracer)
    print(f"\033[32m模型 >\033[0m {out.text}")

    print("\n\033[1m【4】第 3 轮：修复语法 + 验证 —— 评估器说 done\033[0m")
    turn3 = [
        scripted(calls=[("bash", {"command": "python3 -c \"import core; print('ok')\""})]),
        scripted(calls=[("edit", {"path": "core.py",
                                  "old_text": "def divide(a, b):\n    if b == 0\n        return None\n    return a / b",
                                  "new_text": "def divide(a, b):\n    if b == 0:\n        return None\n    return a / b"})]),
        scripted(calls=[("bash", {"command": "python3 -c \"import core; print('ok')\""})]),
        scripted("除零保护已就位，验证通过。"),
    ]
    out = run_turn(get_provider(demo_script=turn3), h, executor, tracer)
    print(f"\033[32m模型 >\033[0m {out.text}")

    print("\n\033[1m【5】目标的状态轨迹（从事件日志重建）\033[0m")
    for ev in sess.events():
        if ev.type.startswith("goal/"):
            d = ev.data
            if ev.type == EV_GOAL_EVALUATE:
                print(f"  \033[90m#{ev.seq:>3}\033[0m evaluate  round={d.get('round')} "
                      f"verdict=\033[35m{d['verdict']}\033[0m  {d['reason'][:40]}")
            elif ev.type in (EV_GOAL_BLOCKED, EV_GOAL_COMPLETE):
                mark = "●" if ev.type == EV_GOAL_COMPLETE else "◼"
                print(f"  \033[90m#{ev.seq:>3}\033[0m {mark} {d['status']:<9} {d['reason'][:50]}")
            else:
                print(f"  \033[90m#{ev.seq:>3}\033[0m start    round={d['round']}  {d['statement'][:40]}")
    print("\033[90m  注意评估器和 continue 的轮次标记：goal 的轨迹在日志里，进程重启也不丢。\033[0m")

    print("\n\033[1m【6】complete 之后：goal 的 prompt 段消失了\033[0m")
    print(f"  {len(_goal_text(h))} 字符 —— 目标已完成，不再占用模型的注意力。")

    print("\n\033[1m【7】分工检查\033[0m")
    print("  \033[33mHarness 管\033[0m：目标保存、状态保存（4 种状态）、预算（max_rounds）、")
    print("           停止条件（done/blocked/budget）、checkpoint（事件日志）")
    print("  \033[36m模型管\033[0m：下一步做什么、是否修改计划、如何解决问题")
    print("\033[90m  对照禁止的写法：while not done: 里面写满 if 判断任务内容。\033[0m")

    print("\n\033[90m" + "─" * 68)
    print("Goal 是持久状态：关掉终端，目标还活在 session.jsonl 里。")
    print("Goal Loop = turn 循环外面再套一层：做 → 观察 → 评估 → 继续/收工。")
    print("评估由模型（评估器）产生判定，Harness 只执行生命周期规则。\033[0m")
