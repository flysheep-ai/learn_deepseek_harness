...
这一章是 s09 的自然延伸，也是全课程最需要警惕"替模型思考"的一章：

    Harness 提供协作机制（spawn / send / receive / status）。
    Harness 不替模型编写协作策略（先派谁、派几个、怎么收尾）。

子 Agent 这次是**持续存活**的：可以来回对话，不再一次性返回。
...
def demo(debug: bool) -> None:
    ...
    (cwd / "core.py").write_text(
        "\n".join(["def divide(a, b):", "    return a / b", "", "def load(path):",
                   "    return open(path).read()"]), encoding="utf-8")

    sess = Session(path=cwd / "session.jsonl")
    sess.append(EV_SESSION_START, {"cwd": str(cwd)})

    h = build_harness("minimal", cwd, sess, tracer,
                      lambda: get_provider(demo_script=[scripted("（未使用）")]),
                      get_provider(demo_script=[scripted("（摘要略）")]),
                      lambda *a: True, skills_root)
    h.rt.project_notes = load_project_notes(cwd)
    executor = ToolExecutor(h.tools, h.bus)

    print("\033[1m【1】模型的协作计划（demo 里由脚本假扮，真实场景是模型自己写的）\033[0m")
    print("\033[90m  Harness 里没有任何一行代码写着「先派 explorer、再派 editor、最后 reviewer」。")
    print("  这些步骤完全是模型在 turn 里自己决定的。\033[0m")

    q = "查一下 divide(0, 0) 会怎样，如果有问题就修复，然后找个人审查一下。"
    print(f"\033[36m你 > \033[0m{q}")
    h.inbox.put(q)

    script = [
        # ① 模型决定：先派一个 explorer 查问题
        scripted(calls=[("spawn_agent", {
            "agent": "explorer",
            "task": "查一下 core.py 的 divide 函数在 b=0 时会怎样，给出文件行号和结论。",
            "persistent": True})]),
        # ② 模型决定：收一下 explorer 的结论（receive 会触发它开工）
        scripted(calls=[("receive", {})]),
        # ③ 模型决定：拿到结论了，派 editor 去修复
        scripted(calls=[("spawn_agent", {
            "agent": "editor",
            "task": "修复 core.py 的 divide 对 b=0 的处理。",
            "persistent": True})]),
        scripted(calls=[("send_message", {
            "agent": "editor",
            "message": "explorer 说 divide 在 b=0 时抛 ZeroDivisionError（core.py:2）。"
                       "请给 divide 加除零保护，改成返回 None 并打印一行警告。"})]),
        # ④ 模型决定：收 editor 的结果，再派 reviewer 审查
        scripted(calls=[("receive", {})]),
        scripted(calls=[("spawn_agent", {
            "agent": "reviewer",
            "task": "审查 core.py 最新的 divide 实现有没有问题。",
            "persistent": True})]),
        scripted(calls=[("send_message", {
            "agent": "reviewer",
            "message": "editor 已经改了 divide（除零返回 None）。请检查这个改动。"})]),
        scripted(calls=[("receive", {})]),
        scripted(calls=[("list_agents", {})]),
        scripted("divide 已加除零保护（editor 完成，reviewer 确认没有新问题），任务结束。"),
    ]

    # 团队成员的处理脚本：每个成员第一次 run_once 时的"模型"
    member_replies = {
        "explorer": [
            scripted(calls=[("read", {"path": "core.py"})]),
            scripted("结论：core.py:2 的 divide 在 b=0 时抛 ZeroDivisionError。需要加除零保护。"),
        ],
        "editor": [
            scripted(calls=[("read", {"path": "core.py"})]),
            scripted(calls=[("edit", {"path": "core.py",
                                      "old_text": "def divide(a, b):\n    return a / b",
                                      "new_text": "def divide(a, b):\n    if b == 0:\n        print('warning: b=0')\n        return None\n    return a / b"})]),
            scripted("已给 divide 加除零保护：b=0 时打印警告并返回 None。"),
        ],
        "reviewer": [
            scripted(calls=[("read", {"path": "core.py"})]),
            scripted("检查了新的 divide：逻辑正确。注意 print 警告可能更该用 logging —— 但这不是 bug，可以后续统一处理。"),
        ],
    }
    ...
    def member_provider():
        return get_provider(demo_script=member_replies.get(state["current"], [scripted("（空）")]))

    orig_run_once = MemberAgent.run_once

    def patched_run_once(self):
        state["current"] = self.name
        try:
            return orig_run_once(self)
        finally:
            state["current"] = None

    MemberAgent.run_once = patched_run_once    # demo 专用补丁
    # 卸载后用 demo 工厂重新装上 subagent 插件
    h.unload("subagent")
    h.use(SubagentPlugin(member_provider, tracer))

    out = run_turn(get_provider(demo_script=script), h, executor, tracer)
    print(f"\033[32m模型 >\033[0m {out.text}")

    print("\n\033[1m【2】最终代码（三个 Agent 协作的产物）\033[0m")
    for line in (cwd / "core.py").read_text(encoding="utf-8").splitlines():
        print(f"  \033[90m{line}\033[0m")

    print("\n\033[1m【3】每个成员留下了什么\033[0m")
    members = h.services.get("agents", {})
    for name, m in members.items():
        print(f"  \033[36m{name:<10}\033[0m status={m.status}  "
              f"steps={[w['steps'] for w in m.work_history]}  "
              f"outbox={len(m.outbox)} 条")
        if m.outbox:
            print(f"         最后产出：{m.outbox[-1][:80]}…")

    print("\n\033[1m【4】模型全程决定的事 vs Harness 全程提供的事\033[0m")
    print("  \033[33m模型决定\033[0m：先派谁、用 persistent 还是一次性、什么时候 send、")
    print("           什么时候 receive、什么时候收尾")
    print("  \033[36mHarness 提供\033[0m：spawn / send / receive / list 四个动词，")
    print("           以及每个成员的独立上下文与受限工具集")
    print("\033[90m  对照禁止的写法：if task_type == 'research': research_agent() ……\033[0m")

    print("\n\033[90m" + "─" * 68)
    print("Harness 提供协作机制，不替模型编写协作策略。")
    print("这一章没有任何新增的事件类型 —— 团队状态本身就是 session 里的工具结果，")
    print("成员的状态由他们自己的事件日志承载。\033[0m")
...
def main() -> None:
    ...
    print(f"\033[1ms16 — Agent Team\033[0m  \033[90mprofile={profile}\033[0m")
    print_harness(h)
    print(f"\n\033[90m子 Agent 角色：{', '.join(SUBAGENT_PRESETS)}；"
          f"/agents 看团队，日志 {log_path}；q 退出\033[0m\n")
