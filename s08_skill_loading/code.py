#!/usr/bin/env python3
"""s08 — Skill Loading（渐进式披露）

    启动时 prompt 里只有目录（约 200 字符）：
        git-workflow  : 本团队的 git 分支、提交与 PR 规范
        python-style  : 本项目的 Python 代码规范
        debugging     : 排查失败测试和运行时错误的系统方法

    模型判断需要时才调用（约 500 字符，且只有这一份）：
        skill("debugging")  ──▶  完整正文注入上下文

这一章回答：**知识应该提前塞进 prompt，还是等模型要的时候再给？**

答案是后者，而且这不只是省 token —— 全塞进去会让模型的注意力
被 90% 用不上的内容稀释。

关键机制叫 **Progressive Disclosure（渐进式披露）**：

    先给目录（便宜、总是在），再按需给正文（贵、只在需要时）。
...
def demo(debug: bool) -> None:
    global WORKSPACE, SKILLS, INBOX, RT
    ...
    # ── 账算给你看 ───────────────────────────────────────────────
    bodies = {n: len(SKILLS.get(n).body) for n in SKILLS.names()}
    catalog_size = len(_skills(RT) or "")
    total_body = sum(bodies.values())
    print("\033[1m【1】目录 vs 全文：渐进式披露省下了什么\033[0m")
    for n, size in bodies.items():
        print(f"  \033[36m{n:<14}\033[0m 正文 {size:>5} 字符   目录里只占 "
              f"{len(n) + len(SKILLS.get(n).description) + 4:>3} 字符")
    print(f"  \033[90m{'─' * 56}\033[0m")
    print(f"  全部塞进 prompt：\033[31m{total_body} 字符/次请求\033[0m")
    print(f"  只放目录：      \033[32m{catalog_size} 字符/次请求\033[0m"
          f"  （{catalog_size * 100 // max(total_body, 1)}%）")

    # ── 第一轮：一个用不上技能的问题 ─────────────────────────────
    print("\n\033[1m【2】一个用不上技能的问题 —— 模型不该加载任何东西\033[0m")
    q1 = "config.py 里 TIMEOUT 是多少？"
    print(f"\033[36m你 > \033[0m{q1}")
    INBOX.put(q1)
    out1 = run_turn(get_provider(demo_script=[
        scripted(calls=[("read", {"path": "config.py"})]),
        scripted("TIMEOUT = 30。"),
    ]), session, executor, RT, INBOX, tracer)
    print(f"\033[32m模型 >\033[0m {out1.text}")
    print(f"\033[90m已加载技能：{RT.loaded_skills or '（无）—— 正是我们想要的'}\033[0m")

    # ── 第二轮：一个需要技能的问题 ───────────────────────────────
    print("\n\033[1m【3】一个需要技能的问题 —— 模型自己决定去取\033[0m")
    q2 = "测试挂了，帮我按团队的方法排查一下"
    print(f"\033[36m你 > \033[0m{q2}")
    INBOX.put(q2)
    before = len(derive_messages(session))
    out2 = run_turn(get_provider(demo_script=[
        # 模型看到目录里有 debugging，自己决定加载
        scripted(calls=[("skill", {"name": "debugging"})]),
        scripted(calls=[("bash", {"command": "python3 -m pytest -x"})]),
        scripted("按 debugging 技能的第 1 步：先复现。这个工作区里没有测试文件，"
                 "所以 pytest 收集为空 —— 失败不在代码里，在于测试根本没被发现。"),
    ]), session, executor, RT, INBOX, tracer)
    print(f"\033[32m模型 >\033[0m {out2.text}")

    # ── 注入路径 ─────────────────────────────────────────────────
    print("\n\033[1m【4】技能正文是怎么进上下文的\033[0m")
    for ev in session.events():
        if ev.type == EV_SKILL_LOAD:
            print(f"  \033[90m#{ev.seq:>3}\033[0m \033[33mskill/load\033[0m   "
                  f"log-only：记下「为什么这一轮突然多了一千多字」")
        if ev.type == EV_USER_MESSAGE and ev.data.get("source") == "skill":
            print(f"  \033[90m#{ev.seq:>3}\033[0m \033[36muser/message\033[0m "
                  f"SURFACE，source=skill，{len(ev.data['content'])} 字符 ← 正文在这里")
    print("\033[90m  正文没有走 tool 结果，而是走 inbox 注入成 user 消息。")
    print("  因为技能是**指令**，不是某次调用的**观察**。\033[0m")

    print("\n\033[1m【5】重复加载会被挡掉\033[0m")
    print(f"  {run_skill('debugging')}")

    print("\n\033[90m" + "─" * 66)
    print("Progressive Disclosure 的两半，缺一不可：")
    print("  · 目录必须**总是**在 —— 否则模型永远想不到去要")
    print("  · 正文必须**按需**给 —— 否则目录就没意义了")
    print(f"这一轮下来，上下文因为技能加载增长了 {len(derive_messages(session)) - before} 条消息，")
    print("但前一轮那个简单问题一个字都没多付。\033[0m")
...
def main() -> None:
    ...
    print("\033[1ms08 — Skill Loading\033[0m")
    print(f"\033[90m技能：{', '.join(SKILLS.names())}（只有目录常驻 prompt）\033[0m")
    print(f"\033[90m日志 {log_path}；/prompt 看组装结果，/tree 看结构，q 退出\033[0m\n")
    ...
    while True:
        ...
        print(f"\033[90m[turn {out.turn} · {out.steps} steps · {out.reason}"
              f" · 已加载技能 {RT.loaded_skills or '无'}]\033[0m\n")
